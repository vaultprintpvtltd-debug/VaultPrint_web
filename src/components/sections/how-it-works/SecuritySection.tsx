...
const layers: ProtectionLayer[] = [
  ...
  },
  {
    ...
    icon: <Trash2 className="text-blue-400" size={24} />
  }
]
...
export default function SecuritySection() {
  return (
    <section className="relative py-24 bg-slate-950 text-white overflow-hidden">
      ...
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-brand-blue/10 rounded-full filter blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-indigo-500/10 rounded-full filter blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-400 w-fit text-xs font-semibold uppercase tracking-wider">
              <ShieldCheck size={14} />
              Bank-grade security
            </div>

            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-white leading-tight">
              Your file stays private. <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Always.</span>
            </h2>
            ...
            <div className="hidden lg:block h-px bg-gradient-to-r from-blue-500/30 to-transparent my-4" />
          </div>
          ...
          <div className="lg:col-span-7 grid sm:grid-cols-2 gap-6">
            {layers.map((layer, index) => {
              ...
              return (
                <div
                  key={layer.title}
                  className={`p-6 rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-sm transition-all duration-300 hover:bg-white/[0.04] hover:border-white/10 ${
