    ...
    >
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-4 z-[2]">
          ...
          >
            <div
              className={`hamburger-line w-6 h-[2px] bg-current transition-[transform,opacity,margin] duration-300 ease-linear origin-center ${
          ...
          <button
            type="button"
            onClick={onCtaClick}
            className="card-nav-cta-button hidden md:inline-flex border-0 rounded-full px-5 items-center h-full font-semibold cursor-pointer transition-colors duration-300 hover:opacity-90 active:scale-95"
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="nav-card-label font-display font-semibold tracking-tight text-[18px] md:text-[22px] mb-2">
                {item.label}
              </div>
              <div className="nav-card-links mt-auto flex flex-col gap-1.5">
                {item.links?.map((lnk, i) => (
                  <a
                    key={`${lnk.label}-${i}`}
                    className="nav-card-link inline-flex items-center gap-2 no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] font-medium"
                    href={lnk.href}
                    aria-label={lnk.ariaLabel}
                  >
                    <ArrowUpRight size={16} className="nav-card-link-icon shrink-0" aria-hidden="true" />
                    {lnk.label}
                  </a>
                ))}
              </div>
